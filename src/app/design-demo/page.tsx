...
export default function DesignDemoPage() {
  ...
  // Auto-advance logic (can be paused if user manually interacts, but for now simple loop)
  // User asked to be able to go back and forth, so I will make dots clickable and slow down the timer or just rely on manual?
  // "Set it so I can go back and forth" -> Manual control is key.
  // I'll keep the auto-timer but make it longer (4s) so user has time, and clicking dots resets it.
  useEffect(() => {
  ...
  return (
    <div className="min-h-[100dvh] bg-white text-gray-900 font-sans flex flex-col items-center justify-between p-6 relative overflow-hidden selection:bg-gray-100">
      ...
      <main className="relative z-10 flex-1 w-full flex flex-col justify-center max-w-md mx-auto h-[60vh]">
        ...
        <div
          className={`absolute inset-0 flex flex-col items-center justify-center transition-all duration-700 ease-in-out ${currentScreen === 1 ? 'opacity-100 translate-x-0 blur-0' : 'opacity-0 translate-x-8 blur-sm pointer-events-none'
            // Logic to handle direction could be complex, keeping simple fade/slide
            // A simplified class toggling for 3 screens:
            // Active: opacity-100
            // Inactive: opacity-0
        ...
        >
          <div className="w-full max-w-sm text-center">
            ...
            <button
              className="w-full bg-gray-900 hover:bg-black active:scale-95 text-white font-bold h-14 rounded-2xl flex items-center justify-center gap-3 transition-all text-base shadow-lg"
            >
      ...
      {/* Footer Elements (Only visible on Screen 0 & 1) */}
